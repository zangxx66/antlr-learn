...
//B.2.2 Types
type_: base_type ('?' | rank_specifier | '*')*;
...
tuple_element: type_ identifier?;
...
type_argument_list: '<' type_ ( ',' type_)* '>';

//B.2.4 Expressions
argument_list: argument ( ',' argument)*;

argument: (identifier ':')? refout = (REF | OUT | IN)? (
		VAR
		| type_
	)? expression;
...
conditional_expression:
	null_coalescing_expression (
		'?' throwable_expression ':' throwable_expression
	)?;
...
primary_expression_start:
	...
	)? block								# anonymousMethodExpression
...
throwable_expression: expression | throw_expression;
...
indexer_argument: (identifier ':')? expression;
...
member_initializer: (identifier | '[' expression ']') '=' initializer_value ; // C# 6
...
isTypePatternArm: identifier ':' expression;
...
anonymous_function_body: throwable_expression | block;
...
combined_join_clause:
	JOIN type_? identifier IN expression ON expression EQUALS expression (
		INTO identifier
	)?;

orderby_clause: ORDERBY ordering (',' ordering)*;

ordering: expression dir = (ASCENDING | DESCENDING)?;
...
local_function_header:
	local_function_modifiers? return_type identifier type_parameter_list? OPEN_PARENS
		formal_parameter_list? CLOSE_PARENS type_parameter_constraints_clauses?;

local_function_modifiers: (ASYNC | UNSAFE) STATIC?
	| STATIC (ASYNC | UNSAFE);
...
typed_member_declaration: (REF | READONLY REF | REF READONLY)? type_ (
...
parameter_modifier: REF | OUT | IN | REF THIS | IN THIS | THIS;
...
event_accessor_declarations:
	attributes? (
		ADD block remove_accessor_declaration
		| REMOVE block add_accessor_declaration
	);
...
conversion_operator_declarator: (IMPLICIT | EXPLICIT) OPERATOR type_ OPEN_PARENS arg_declaration
		CLOSE_PARENS;
...
interface_member_declaration:
	attributes? NEW? (
		...
		| UNSAFE? VOID identifier type_parameter_list? OPEN_PARENS formal_parameter_list?
...
global_attribute_target: keyword | identifier;
...
attribute_target: keyword | identifier;
...
pointer_type: (simple_type | class_type) (rank_specifier | '?')* '*'
	| VOID '*';
...
stackalloc_initializer:
	...
	| STACKALLOC type_? '[' expression? ']' OPEN_BRACE expression (
		',' expression
	)* ','? CLOSE_BRACE;
...
struct_definition: (READONLY | REF)? STRUCT identifier type_parameter_list? struct_interfaces?
...
enum_definition: ENUM identifier enum_base? enum_body ';'?;

delegate_definition:
	DELEGATE return_type identifier variant_type_parameter_list? OPEN_PARENS formal_parameter_list?
		CLOSE_PARENS type_parameter_constraints_clauses? ';';
...
field_declaration: variable_declarators ';';
...
constant_declaration: CONST type_ constant_declarators ';';
...
method_member_name: (identifier | identifier '::' identifier) (
